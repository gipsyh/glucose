...
void Solver::simpleUncheckEnqueue(Lit p, CRef from)
{
	...
	assigns[var(p)] = lbool(
		!sign(p)); // this makes a lbool object whose value is sign(p)
			   ...
			   vec<CRef> &reason_clause, bool True_confl)
{
	...
	do {
		if (confl != CRef_Undef) {
			...
			for (int j = (p == lit_Undef && True_confl == false) ?
					     0 :
					     1;
			     j < c.size(); j++) {
				Lit q = c[j];
...
void Solver::simplifyLearnt(Clause &c)
{
	...
	if (confl != CRef_Undef || True_confl == true) {
		...
		simpleAnalyze(confl, simp_learnt_clause, simp_reason_clause,
			      True_confl);
			...
			if (!removed(cr)) {
				...
				if (sat) {
					...
					if (c.simplified() || c.lbd() > 3) {
						attachClause(cr);
						permanentLearntsReduced.push(
							cr); //[cj++] = permanentLearnts[ci];
					...
					if (beforeSize > afterSize) {
						if (afterSize <= 3) {
							nbGoodReduce++;
							if (afterSize > 1)
								parallelExportClauseDuringSearch(
									c);
							if (afterSize == 2) {
								nbSize2++;
							}
